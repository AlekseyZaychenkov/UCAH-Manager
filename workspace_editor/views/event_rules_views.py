...
@login_required
def event_rules_view(request, workspace_id=None):
...
def __event_rules_request_handler(request, workspace):
    if request.POST['action'] == "save_event_rules":
    ...
    for i in range(1, 7):
        ...
        for j in range(0, 7):
            if j < len(list_for_current_priority):
                ...
                posting_time_and_form = {"posting_time_obj": posting_time,
                                             "posting_time_form":PostingTimeEditForm(
