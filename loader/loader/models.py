...
class PostEntry(Model):
    ...
    original_blog_url            = columns.Text(max_length=250, required=True)
    ...
    downloated_date              = columns.Text(max_length=30, required=True, index=True)
    ...
    description                  = columns.Text(required=False)
    storage                      = columns.Text(max_length=512, required=False)
